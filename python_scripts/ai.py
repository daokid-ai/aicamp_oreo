...
def yolo_forward(net, LABELS, image, validated_img_path, confidence_level, save_image):
    """
    ...
    if save_image == 'True':
        print("Saving {}".format(validated_img_path))
        yolo_save_img(image, class_ids, boxes, labels, confidences, colors, validated_img_path)
...
def yolo_show_img(image, class_ids, boxes, labels, confidences, colors):
    ...
    resize = resizewithaspectratio(image, height=720)  # Resize by width OR
...
def yolo_pred(image_path, validated_folder_path, names_path, cfg_path, weight_path, confidence_lvl, save_image):
    ...
    validated_img_path = os.path.join(validated_folder_path, "python_predictions.jpg")

    (class_ids, labels, boxes, confidences) = yolo_forward(
        net, labels, image, validated_img_path, confidence_lvl, save_image)

    yolo_show_img(image, class_ids, boxes, labels, confidences, colors)


def yolo_pred_list(image_folder_path, validated_folder_path, names_file, cfg_file, weight_file, confidence_level,
                   threshold, save_image):

    all_paths = os.listdir(image_folder_path)
    all_paths.sort()

    image_paths = [os.path.join(image_folder_path, f) \
        for f in all_paths if '.jpg' in f or '.jpeg' in f or '.png' in f]

    all_validated_paths = [os.path.join(validated_folder_path, "v_{}".format(confidence_level) + "_{}".format(f)) \
        for f in all_paths if '.jpg' in f or '.jpeg' in f or '.png' in f]
    ...
    for image_path in image_paths:
        ...
        image = cv2.imread(image_path)
        (class_ids, labels, boxes, confidences) = yolo_forward(
            net, LABELS, image, all_validated_paths[file_index], confidence_level, save_image)
        result = {
...
def yolo_video(name_path, cfg_path, weight_path, validated_folder_path):
    # get the net using cfg and weight
    net = get_yolo_net(cfg_path, weight_path)
    ...
    while True:
        ...
        validated_img_path = os.path.join(validated_folder_path, "python_predictions.jpg")
        (class_ids, labels, boxes, confidences) = yolo_forward(
            net, LABELS, image, validated_img_path, confidence_level=0.3)
